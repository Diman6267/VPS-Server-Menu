...
Логично. Если мы добавили строку в .bashrc, то при удалении её нужно обязательно вычищать, иначе при каждом входе в систему ты будешь видеть эту раздражающую ошибку -bash: server-menu: command not found.

Вот обновленный блок для твоего uninstall.sh. Он найдет и удалит именно ту строку, которую мы добавили.

Обновленный uninstall.sh
Добавь этот блок в свой скрипт удаления на GitHub:

Bash
# ... (удаление ссылок и папок) ...
